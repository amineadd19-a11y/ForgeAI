/**
 * Approximate provider cost estimates (USD) for margin / observability.
 * These are operational estimates — not billing truth. Credits remain the
 * product currency; this layer is for internal cost awareness only.
 *
 * Prices are rough public-list order-of-magnitude figures and can lag reality.
 */
...
/** USD per 1M tokens — approximate list prices for common models */
...
export function estimateProviderCostUsd(
  ...
  return {
    ...
    model,
    rates: { inputPerMTok: rates.input, outputPerMTok: rates.output },
  };
}
