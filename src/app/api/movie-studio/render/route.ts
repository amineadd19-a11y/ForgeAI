...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.FAL_KEY) {
    return NextResponse.json(
      {
        error:
          "Image/video rendering is not configured. Add FAL_KEY to the server environment to enable media generation.",
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid media request", code: "INVALID_REQUEST", requestId, creditsCharged: 0 },
      { status: 400 }
    );
  }
  ...
  const cost =
    parsed.data.type === "image"
      ? calculateCost("generate", "standard")
      : Math.max(calculateCost("generate", "advanced") * 3, 12);
  const balance = await getBalance(user.id);
  if (balance < cost) {
    return NextResponse.json(
      {
        error: `This ${parsed.data.type} generation costs ${cost} credits. You have ${balance}.`,
  ...
  if (parsed.data.type === "video" && !parsed.data.imageUrl) {
    return NextResponse.json(
      {
        error: "Video generation requires a rendered scene image first.",
        code: "IMAGE_REQUIRED",
        requestId,
        creditsCharged: 0,
      },
  ...
  await prisma.aiRequest.create({
    data: {
      ...
      status: "PENDING",
      creditsCharged: 0,
      metadata: { feature: "movie-studio-render", type: parsed.data.type },
    },
  });
  ...
  try {
    ...
    await prisma.aiRequest.update({
      ...
      data: {
        ...
        metadata: { endpoint, mediaUrl, type: parsed.data.type },
      },
    });
    return NextResponse.json({
      requestId,
      ...
      url: mediaUrl,
      usage: { credits: charge.success ? cost : 0, remaining: charge.balanceAfter },
    });
  } catch (error) {
    await prisma.aiRequest.update({
      where: { requestId },
      data: {
        status: "FAILED",
        errorMessage:
          error instanceof Error ? error.message.slice(0, 500) : "Media generation failed",
        completedAt: new Date(),
      },
    });
    return NextResponse.json(
