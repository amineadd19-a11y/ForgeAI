      ...
      <header className="border-b border-zinc-800 sticky top-0 z-20 bg-zinc-950/90 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1 text-sm overflow-x-auto">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-md px-3 py-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 transition"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-sm text-zinc-400 truncate max-w-[140px] hidden sm:inline">
              {session.user.email}
            </span>
            <LogoutButton />
          </div>
