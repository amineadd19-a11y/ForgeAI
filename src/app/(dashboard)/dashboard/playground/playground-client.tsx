...
function inferProvider(modelId: string, catalog: ModelCatalogEntry[]): ModelProviderId | "unknown" {
...
export function PlaygroundClient({
  ...
  const allowedModels = useMemo(() => getModelsForPlan(planTier), [planTier]);
  const defaultModel = allowedModels[0]?.id || "gpt-4o-mini";
  ...
  // Keep selected model within the allowed set if plan data changes
  useEffect(() => {
  ...
  const selectedProvider = inferProvider(model, allowedModels);
  const catalogGroups = useMemo(() => groupByProvider(allowedModels), [allowedModels]);

  const creditHint = complexity === "basic" ? 1 : complexity === "standard" ? 2 : 5;
  ...
  async function onGenerate(e?: React.FormEvent) {
    ...
    if (!apiKey.trim() || !prompt.trim() || !model) return;
    ...
    try {
      ...
      // Abort reader when user stops — closes the HTTP body so the server sees cancel()
      const onAbort = () => {
        try {
          void reader.cancel();
        } catch {
  ...
  const noModels = allowedModels.length === 0;
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Models shown for your <span className="text-zinc-200">{planName}</span> plan. Server
            still enforces allow-lists. History:{" "}
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 px-3 py-2 text-xs text-zinc-400">
          <div>
            Plan: <span className="text-zinc-200 font-medium">{planName}</span>
            {" · "}
            Provider:{" "}
          ...
          <div className="mt-1 text-zinc-500">
            ...
            Partial streams are never mixed across providers. Stopping cancels the HTTP body so the
            server does not charge credits.
          </div>
        </div>
      </div>
      ...
      {noModels ? (
        <div className="rounded-xl border border-amber-900/50 bg-amber-950/20 p-4 text-amber-200 text-sm">
          No models are available for your plan. Contact support or upgrade your subscription.
        </div>
      ) : (
        <form onSubmit={onGenerate} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">
                Only models on your plan. Unauthorized models still return 403 from the API.
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex gap-2 ml-auto">
              ...
              <button
                type="submit"
                disabled={loading || overLimit || !prompt.trim() || !apiKey.trim() || !model}
